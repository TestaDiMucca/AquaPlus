...
export default class ShowCard extends Component {
  componentWillUnmount() {
    ...
    if (user && this.isViewedEnough()) {
      ...
      const url = `${SERVER}/watched/${encodeURIComponent(
        user
      )}/${encodeURIComponent(useName)}`;
      fakexios.post(url, {}).catch((err) => {});
    }
  }
  ...
  isViewedEnough() {
    ...
    if (!vidElement || !vidElement.currentTime || !vidElement.duration)
      return false;
  ...
  render() {
    ...
    const url =
      `${SERVER}/player/${encodeURIComponent(show)}/${encodeURIComponent(
        filename
      )}` + addQuery;
    return (
      <div class={style.viewer}>
        <div onClick={onClose} class={style.closeButton}>
          ×
        </div>
        {show} {filename}
        <video
          id="video-player"
          class={style.viewerVideo}
          src={url}
          controls
          type={isFirefox() ? 'video/webm' : 'video/mp4'}
        />
      </div>
    );
  }
